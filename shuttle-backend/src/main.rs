...
async fn main(
    ...
    // Initialize chat service (using injected qdrant_client)
    let openai_api_key = std::env::var("OPENAI_API_KEY")
        .unwrap_or_else(|_| "sk-mock-key-for-testing".to_string());
    
    // Create services with Qdrant client
    let openai_api_key_2 = openai_api_key.clone();
    
    let chat_service = std::sync::Arc::new(ChatService::new(qdrant_client, openai_api_key));
    ...
    let rag_system = std::sync::Arc::new(RAGSystem::new(qdrant_client_for_rag, openai_api_key_2));
